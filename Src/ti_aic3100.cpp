...
//********** AIC3100 PAGE 0
        // @formatter:off
// pg,reg, W1_allowed, W1_required, reset_val,  "name",   prev_val, curr_val
    MAKE_AIC( 0,   0, 0xFF, 0x00, 0x00, "psr0",      0, 0x00),   // 0=0.0  must include Page_Select for all pages in use
    MAKE_AIC( 0,   1, 0x01, 0x00, 0x00, "s_rst",     0, 0x00),   // 1=0.1
    MAKE_AIC( 0,   2, 0x00, 0xff, 0x01, "rsv",       0, 0x01),   // actual: 0:02  0x01 not 0x00 // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,   3, 0x02, 0x00, 0x66, "ot_flg",    0, 0x66),   // X bits D2, D5,D6 set in practice-- actual: 0x66 not 0x02
    MAKE_AIC( 0,   4, 0x0F, 0x00, 0x00, "clk_mux",   0, 0x00),   // 4=0.4
    MAKE_AIC( 0,   5, 0xFF, 0x00, 0x11, "pll_p_r",   0, 0x11),
    MAKE_AIC( 0,   6, 0x3F, 0x00, 0x04, "pll_j",     0, 0x04),
    MAKE_AIC( 0,   7, 0x3F, 0x00, 0x00, "pll_dhi",   0, 0x00),
    MAKE_AIC( 0,   8, 0xFF, 0x00, 0x00, "pll_dlo",   0, 0x00),
    MAKE_AIC( 0,   9, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,  10, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,  11, 0xFF, 0x00, 0x01, "dac_n",     0, 0x01),
    MAKE_AIC( 0,  12, 0xFF, 0x00, 0x01, "dac_m",     0, 0x01),
    MAKE_AIC( 0,  13, 0x03, 0x00, 0x00, "dosr_hi",   0, 0x00),
    MAKE_AIC( 0,  14, 0xFF, 0x00, 0x80, "dosr_lo",   0, 0x80),
    MAKE_AIC( 0,  15, 0xFF, 0x00, 0x80, "dac_idac",  0, 0x80),
    MAKE_AIC( 0,  16, 0x0F, 0x00, 0x08, "dac_mac_en",0, 0x08),
    MAKE_AIC( 0,  17, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,  18, 0xFF, 0x00, 0x01, "adc_n",     0, 0x01),
    MAKE_AIC( 0,  19, 0xFF, 0x00, 0x01, "adc_m",     0, 0x01),
    MAKE_AIC( 0,  20, 0xFF, 0x00, 0x80, "aosr",      0, 0x80),
    MAKE_AIC( 0,  21, 0xFF, 0x00, 0x80, "adc_iadc",  0, 0x80),
    MAKE_AIC( 0,  22, 0xFF, 0x00, 0x04, "adc_prb",   0, 0x04),   // use actual: 0x04 not 0x80 (reserved)
    MAKE_AIC( 0,  23, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,  24, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,  25, 0x07, 0x00, 0x00, "out_mux",   0, 0x00),
    MAKE_AIC( 0,  26, 0xFF, 0x00, 0x01, "out_m",     0, 0x01),
    MAKE_AIC( 0,  27, 0xFD, 0x00, 0x00, "i2s",       0, 0x00),
    MAKE_AIC( 0,  28, 0xff, 0x00, 0x00, "d_off",     0, 0x00),
    MAKE_AIC( 0,  29, 0x3F, 0x00, 0x00, "bclk",      0, 0x00),
    MAKE_AIC( 0,  30, 0xFF, 0x00, 0x01, "bclk_n",    0, 0x01),
    MAKE_AIC( 0,  31, 0xff, 0x00, 0x00, "cdc2_ctl1", 0, 0x00),
    MAKE_AIC( 0,  32, 0xff, 0x00, 0x00, "cdc2_ctl2", 0, 0x00),
    MAKE_AIC( 0,  33, 0xFF, 0x00, 0x00, "b&w_clks",  0, 0x00),
    MAKE_AIC( 0,  34, 0x20, 0x00, 0x00, "i2c_bus",   0, 0x00),
    MAKE_AIC( 0,  35, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,  36, 0x00, 0x00, 0x80, "adc_flg",   0, 0x80),   // use actual: 0x80 not 0x00
    MAKE_AIC( 0,  37, 0x00, 0x00, 0x00, "dac_flg",   0, 0x00),   // 37=0.37
    MAKE_AIC( 0,  38, 0x00, 0x00, 0x00, "dac_flg2",  0, 0x00),
    MAKE_AIC( 0,  39, 0x00, 0x00, 0x00, "ovfl_flg",  0, 0x00),
    MAKE_AIC( 0,  40, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // 40=0.40  reserved-- reset = XX, can1=0x00, must1=0xff
   // dup 41..43
    MAKE_AIC( 0,  44, 0x00, 0x00, 0x00, "dac_ints",  0, 0x00),   // 41=0.44
    MAKE_AIC( 0,  45, 0x00, 0x00, 0x00, "adc_ints",  0, 0x00),
    MAKE_AIC( 0,  46, 0x00, 0x00, 0x00, "dac_ints2", 0, 0x00),
    MAKE_AIC( 0,  47, 0x00, 0x00, 0x00, "adc_ints2", 0, 0x00),
    MAKE_AIC( 0,  48, 0xff, 0x00, 0x00, "int_ctl1",  0, 0x00),
    MAKE_AIC( 0,  49, 0xff, 0x00, 0x00, "int_ctl2",  0, 0x00),
    MAKE_AIC( 0,  50, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,  51, 0xfd, 0x00, 0x02, "gpio_pin",  0, 0x02),   // actual: 0:51  0x02 not 0x00
    MAKE_AIC( 0,  52, 0x00, 0xff, 0x32, "rsv",       0, 0x00),   // actual: 0:52  0x32 not 0x00  //reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,  53, 0x1F, 0x00, 0x12, "d_out",     0, 0x12),   // 50=0.53
    MAKE_AIC( 0,  54, 0xfe, 0x00, 0x03, "d_in",      0, 0x02),   // actual: 0:54  0x03 not 0x02
    MAKE_AIC( 0,  55, 0x00, 0xff, 0x02, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
   // dup 56..59
    MAKE_AIC( 0,  60, 0x3F, 0x00, 0x01, "dac_prb",   0, 0x01),   // 53=0.60
    MAKE_AIC( 0,  61, 0x1F, 0x00, 0x04, "adc_prb",   0, 0x04),
    MAKE_AIC( 0,  62, 0x77, 0x00, 0x00, "pr_ctrl",   0, 0x00),     // Programmable_Instruction_Mode_Control
    MAKE_AIC( 0,  63, 0xFF, 0x00, 0x14, "dac_path",  0, 0x14),   // 56=0.63
    MAKE_AIC( 0,  64, 0x0F, 0x00, 0x0C, "dac_vol",   0, 0x0C),
    MAKE_AIC( 0,  65, 0xFF, 0x00, 0x00, "dac_l_vol", 0, 0x00),
    MAKE_AIC( 0,  66, 0xFF, 0x00, 0x00, "dac_r_vol", 0, 0x00),
    MAKE_AIC( 0,  67, 0x9F, 0x00, 0x00, "hp_det",    0, 0x00),   // 60=0.67
    MAKE_AIC( 0,  68, 0x7F, 0x00, 0x6F, "drc_ctl",   0, 0x6F),   // use actual reset 0x6f (DRC enabled)
    MAKE_AIC( 0,  69, 0x78, 0x00, 0x38, "drc_ctl2",  0, 0x38),
    MAKE_AIC( 0,  70, 0xff, 0x00, 0x00, "drc_ctl3",  0, 0x00),
    MAKE_AIC( 0,  71, 0xbf, 0x00, 0x00, "l_beep",    0, 0x00),
    MAKE_AIC( 0,  72, 0xff, 0x00, 0x00, "r_beep",    0, 0x00),
    MAKE_AIC( 0,  73, 0xff, 0x00, 0x00, "bp_ln_msb", 0, 0x00),
    MAKE_AIC( 0,  74, 0xff, 0x00, 0x00, "bp_ln_mid", 0, 0x00),
    MAKE_AIC( 0,  75, 0xff, 0x00, 0xee, "bp_ln_lsb", 0, 0x00),
    MAKE_AIC( 0,  76, 0xff, 0x00, 0x10, "bp_sin_msb",0, 0x10),
    MAKE_AIC( 0,  77, 0xff, 0x00, 0xd8, "bp_sin_lsb",0, 0xd8),   // 70=0.77
    MAKE_AIC( 0,  78, 0xff, 0x00, 0x7e, "bp_cos_msb",0, 0x7e),
    MAKE_AIC( 0,  79, 0xff, 0x00, 0xe3, "bp_cos_lsb",0, 0xe3),
    MAKE_AIC( 0,  80, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
// pg,reg, W1_allowed, W1_required, reset_val,  "name",   prev_val, curr_val
    MAKE_AIC( 0,  81, 0xBB, 0x00, 0x00, "adc_pwr",   0, 0x00),   // 74=0.81
    MAKE_AIC( 0,  82, 0xF0, 0x00, 0x80, "adc_sm_vol",0, 0x80),
    MAKE_AIC( 0,  83, 0x7F, 0x00, 0x00, "adc_vol",   0, 0x00),
    MAKE_AIC( 0,  84, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,  85, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 0,  86, 0xF0, 0x00, 0x00, "agc_tgt",   0, 0x00),   // 79=0.86
    MAKE_AIC( 0,  87, 0xFE, 0x00, 0x00, "agc_ctl2",  0, 0x00),   // 80=0.87
    MAKE_AIC( 0,  88, 0x7F, 0x00, 0x7F, "agc_maxg",  0, 0x7F),
    MAKE_AIC( 0,  89, 0xFF, 0x00, 0x00, "agc_atak",  0, 0x00),
    MAKE_AIC( 0,  90, 0xFF, 0x00, 0x00, "agc_decy",  0, 0x00),
    MAKE_AIC( 0,  91, 0x1F, 0x00, 0x00, "agc_nois",  0, 0x00),
    MAKE_AIC( 0,  92, 0x0F, 0x00, 0x00, "agc_sig",   0, 0x00),   // 85=0.92
    MAKE_AIC( 0,  93, 0x00, 0x00, 0x00, "agc_gain",  0, 0x00),
    MAKE_AIC( 0,  94, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
   // dup 95..101
    MAKE_AIC( 0, 102, 0xBF, 0x00, 0x00, "adc_meas",  0, 0x00),   // 88=0.102  // discovered by Marc
    MAKE_AIC( 0, 103, 0x7F, 0x00, 0x00, "adc_meas2", 0, 0x00),
    MAKE_AIC( 0, 104, 0x00, 0x00, 0x00, "dc_meas1",  0, 0x00),   // 90=0.104
    MAKE_AIC( 0, 105, 0x00, 0x00, 0x00, "dc_meas2",  0, 0x00),
    MAKE_AIC( 0, 106, 0x00, 0x00, 0x00, "dc_meas3",  0, 0x00),
    MAKE_AIC( 0, 107, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // 93=0.107 // reserved-- reset = XX, can1=0x00, must1=0xff
   // dup 108..115
    MAKE_AIC( 0, 116, 0xf7, 0x00, 0x00, "mdet_vol",  0, 0x00),   // 94=0.116
    MAKE_AIC( 0, 117, 0x80, 0x00, 0xFF, "mdet_gn",   0, 0x00),   // reset: 0xxx xxxx
    MAKE_AIC( 0, 118, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // 96=0.118 // reserved-- reset = XX, can1=0x00, must1=0xff
   // dup 119..127

// ********** AIC3100 PAGE 1
// pg,reg, W1_allowed, W1_required, reset_val,  "name",   prev_val, curr_val
    MAKE_AIC( 1,   0, 0x00, 0x00, 0x01, "psr1",      0, 0x01),   // 97=1.0  // 1:0 reads as 0x01
    MAKE_AIC( 1,   1, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
   // dup 2..29
    MAKE_AIC( 1,  30, 0x03, 0x00, 0x00, "err_ctl",   0, 0x00),   // 99=1.30 // Not Used by init
    MAKE_AIC( 1,  31, 0xDE, 0x04, 0x04, "dp_dr",     0, 0x04),   // 100=1.31
    MAKE_AIC( 1,  32, 0xFE, 0x06, 0x06, "cls_d",     0, 0x06),
    MAKE_AIC( 1,  33, 0xFF, 0x00, 0x3E, "pop_rem",   0, 0x3E),
    MAKE_AIC( 1,  34, 0xFF, 0x00, 0x00, "pga_ramp",  0, 0x00),
    MAKE_AIC( 1,  35, 0xFF, 0x00, 0x00, "dac_rout",  0, 0x00),
    MAKE_AIC( 1,  36, 0xFF, 0x00, 0x7F, "l_vol_hp",  0, 0x7F),   // 105=1.36
    MAKE_AIC( 1,  37, 0xFF, 0x00, 0x7F, "r_vol_hp",  0, 0x7F),
    MAKE_AIC( 1,  38, 0xFF, 0x00, 0x7F, "l_vol_spk", 0, 0x7F),
    MAKE_AIC( 1,  39, 0xFF, 0x00, 0x7F, "r_vol_spk", 0, 0x7F),   // 108=1.39
    MAKE_AIC( 1,  40, 0x7E, 0x02, 0x02, "hpl_dr",    0, 0x02),
    MAKE_AIC( 1,  41, 0x7E, 0x02, 0x02, "hpl_dr",    0, 0x02),   // 110=1.41
    MAKE_AIC( 1,  42, 0x1D, 0x00, 0x00, "spk_dr",    0, 0x00),
    MAKE_AIC( 1,  43, 0x00, 0xff, 0x00, "rsv",       0, 0x00),     // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 1,  44, 0xFF, 0x00, 0x20, "hp_dr",     0, 0x20),     // use actual: 0x20 not 0x00 (debounce 8usec)
    MAKE_AIC( 1,  45, 0x00, 0xff, 0x86, "rsv",       0, 0x00),   // actual: 1:45  0x86 not 0x00   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 1,  46, 0x8B, 0x00, 0x00, "mic_bias",  0, 0x00),   // 115=1.46
    MAKE_AIC( 1,  47, 0xFF, 0x00, 0x80, "mic_gain",  0, 0x80),
    MAKE_AIC( 1,  48, 0xFC, 0x00, 0x00, "adc_in_p",  0, 0x00),   // 117=1.48
    MAKE_AIC( 1,  49, 0xF0, 0x00, 0x00, "adc_in_m",  0, 0x00),   // 118=1.49
    MAKE_AIC( 1,  50, 0xE0, 0x00, 0x00, "adc_in_cm", 0, 0x00),
    MAKE_AIC( 1,  51, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // 120=1.51  // reserved-- reset = XX, can1=0x00, must1=0xff
   // dup 52..127


// ********** AIC3100 PAGE 3
// pg,reg, W1_allowed, W1_required, reset_val,  "name",   prev_val, curr_val
    MAKE_AIC( 3,   0, 0x00, 0x00, 0x03, "psr3",      0, 0x03),   // 121=3.0 3:0 reads as 0x03
    MAKE_AIC( 3,   1, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
   // dup 2..15
    MAKE_AIC( 3,  16, 0xff, 0x00, 0x81, "mclk_div",  0, 0x81),   // 123=3.16
    MAKE_AIC( 3,  17, 0x00, 0xff, 0x81, "rsv",       0, 0x00),   // actual: 3:17  0x81 not 0x00  // reserved-- reset = XX, can1=0x00, must1=0xff
   // dup 18..127
// ********** AIC3100 PAGE 4
// pg,reg, W1_allowed, W1_required, reset_val,  "name",   prev_val, curr_val
    MAKE_AIC( 4,   0, 0x00, 0x00, 0x04, "psr4",      0, 0x04),   // 125=4.0  4:0 reads as 0x04
    MAKE_AIC( 4,   1, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 4,   2, 0xff, 0x00, 0x01, "n0_lpf_h",    0, 0x01),
    MAKE_AIC( 4,   3, 0xff, 0x00, 0x17, "n0_lpf_l",    0, 0x17),
    MAKE_AIC( 4,   4, 0xff, 0x00, 0x01, "n1_lpf_h",    0, 0x01),
    MAKE_AIC( 4,   5, 0xff, 0x00, 0x17, "n1_lpf_l",    0, 0x17),  // 130=4.5
    MAKE_AIC( 4,   6, 0xff, 0x00, 0x7d, "d1_lpf_h",    0, 0x7d),
    MAKE_AIC( 4,   7, 0xff, 0x00, 0xd3, "d1_lpf_l",    0, 0xd3),
    MAKE_AIC( 4,   8, 0xff, 0x00, 0x7f, "n0_iir_h",    0, 0x7f),
    MAKE_AIC( 4,   9, 0xff, 0x00, 0xff, "n0_iir_l",    0, 0xff),
    MAKE_AIC( 4,  10, 0xff, 0x00, 0x00, "iir",         0, 0x00),
   // dup 11..13
    MAKE_AIC( 4,  14, 0xff, 0x00, 0x7f, "n0_bqa_h",    0, 0x7f),
    MAKE_AIC( 4,  15, 0xff, 0x00, 0xff, "n0_bqa_l",    0, 0xff),
    MAKE_AIC( 4,  16, 0xff, 0x00, 0x00, "bqa",         0, 0x00),
   // dup 17..23
    MAKE_AIC( 4,  24, 0xff, 0x00, 0x7f, "n0_bqb_h",    0, 0x7f),
    MAKE_AIC( 4,  25, 0xff, 0x00, 0xff, "n0_bqb_l",    0, 0xff),  // 140=4.25
    MAKE_AIC( 4,  26, 0xff, 0x00, 0x00, "bqb",         0, 0x00),
   // dup 27..33
    MAKE_AIC( 4,  34, 0xff, 0x00, 0x7f, "n0_bqc_h",    0, 0x7f),
    MAKE_AIC( 4,  35, 0xff, 0x00, 0xff, "n0_bqc_l",    0, 0xff),
    MAKE_AIC( 4,  36, 0xff, 0x00, 0x00, "bqc",         0, 0x00),
   // dup 37..43
    MAKE_AIC( 4,  44, 0xff, 0x00, 0x7f, "n0_bqd_h",    0, 0x7f),
    MAKE_AIC( 4,  45, 0xff, 0x00, 0xff, "n0_bqd_l",    0, 0xff),
    MAKE_AIC( 4,  46, 0xff, 0x00, 0x00, "bqd",         0, 0x00),
   // dup 47..53
    MAKE_AIC( 4,  54, 0xff, 0x00, 0x7f, "n0_bqe_h",    0, 0x7f),
    MAKE_AIC( 4,  55, 0xff, 0x00, 0xff, "n0_bqe_l",    0, 0xff),
    MAKE_AIC( 4,  56, 0xff, 0x00, 0x00, "bqe",         0, 0x00),  // 150=4.56
   // dup 57..127


// ********** AIC3100 PAGE 8
// pg,reg, W1_allowed, W1_required, reset_val,  "name",   prev_val, curr_val
    MAKE_AIC( 8,   0, 0x00, 0x00, 0x08, "psr8",      0, 0x08),    // 151=8.0  // 8:0 reads as 0x08
    MAKE_AIC( 8,   1, 0x05, 0x00, 0x00, "dac_mode",  0, 0x00),
    MAKE_AIC( 8,   2, 0xff, 0x00, 0x7f, "dac_bqa_h", 0, 0x7f),
    MAKE_AIC( 8,   3, 0xff, 0x00, 0xff, "dac_bqa_l", 0, 0xff),
    MAKE_AIC( 8,   4, 0xff, 0x00, 0x00, "dac_bqa",   0, 0x00),
   // dup 5..11
    MAKE_AIC( 8,  12, 0xff, 0x00, 0x7f, "dac_bqb_h", 0, 0x7f),
    MAKE_AIC( 8,  13, 0xff, 0x00, 0xff, "dac_bqb_l", 0, 0xff),
    MAKE_AIC( 8,  14, 0xff, 0x00, 0x00, "dac_bqb",   0, 0x00),
   // dup 15..21
    MAKE_AIC( 8,  22, 0xff, 0x00, 0x7f, "dac_bqc_h", 0, 0x7f),
    MAKE_AIC( 8,  23, 0xff, 0x00, 0xff, "dac_bqc_l", 0, 0xff),    // 160=8.23
    MAKE_AIC( 8,  24, 0xff, 0x00, 0x00, "dac_bqc",   0, 0x00),
   // dup 25..31
    MAKE_AIC( 8,  32, 0xff, 0x00, 0x7f, "dac_bqd_h", 0, 0x7f),
    MAKE_AIC( 8,  33, 0xff, 0x00, 0xff, "dac_bqd_l", 0, 0xff),
    MAKE_AIC( 8,  34, 0xff, 0x00, 0x00, "dac_bqd",   0, 0x00),
   // dup 35..41
    MAKE_AIC( 8,  42, 0xff, 0x00, 0x7f, "dac_bqe_h", 0, 0x7f),
    MAKE_AIC( 8,  43, 0xff, 0x00, 0xff, "dac_bqe_l", 0, 0xff),
    MAKE_AIC( 8,  44, 0xff, 0x00, 0x00, "dac_bqe",   0, 0x00),
   // dup 45..51
    MAKE_AIC( 8,  52, 0xff, 0x00, 0x7f, "dac_bqf_h", 0, 0x7f),
    MAKE_AIC( 8,  53, 0xff, 0x00, 0xff, "dac_bqf_l", 0, 0xff),
    MAKE_AIC( 8,  54, 0xff, 0x00, 0x00, "dac_bqf",   0, 0x00),   // 170=8.54
   // dup 55..61
    MAKE_AIC( 8,  62, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 8,  63, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 8,  64, 0xff, 0x00, 0x00, "pga_gn_h",  0, 0x00),
    MAKE_AIC( 8,  65, 0xff, 0x00, 0x00, "pga_gn_l",  0, 0x00),
    MAKE_AIC( 8,  66, 0xff, 0x00, 0x7f, "dac_rbqa_h",0, 0x7f),
    MAKE_AIC( 8,  67, 0xff, 0x00, 0xff, "dac_rbqa_l",0, 0xff),
    MAKE_AIC( 8,  68, 0xff, 0x00, 0x00, "dac_rbqa",  0, 0x00),
   // dup 69..75
    MAKE_AIC( 8,  76, 0xff, 0x00, 0x7f, "dac_rbqb_h",0, 0x7f),
    MAKE_AIC( 8,  77, 0xff, 0x00, 0xff, "dac_rbqb_l",0, 0xff),
    MAKE_AIC( 8,  78, 0xff, 0x00, 0x00, "dac_rbqb",  0, 0x00),   // 180=8.78
   // dup 79..85
    MAKE_AIC( 8,  86, 0xff, 0x00, 0x7f, "dac_rbqc_h",0, 0x7f),
    MAKE_AIC( 8,  87, 0xff, 0x00, 0xff, "dac_rbqc_l",0, 0xff),
    MAKE_AIC( 8,  88, 0xff, 0x00, 0x00, "dac_rbqc",  0, 0x00),
   // dup 89..95
    MAKE_AIC( 8,  96, 0xff, 0x00, 0x7f, "dac_rbqd_h",0, 0x7f),
    MAKE_AIC( 8,  97, 0xff, 0x00, 0xff, "dac_rbqd_l",0, 0xff),
    MAKE_AIC( 8,  98, 0xff, 0x00, 0x00, "dac_rbqd",  0, 0x00),
   // dup 99..105
    MAKE_AIC( 8, 106, 0xff, 0x00, 0x7f, "dac_rbqe_h",0, 0x7f),
    MAKE_AIC( 8, 107, 0xff, 0x00, 0xff, "dac_rbqe_l",0, 0xff),
    MAKE_AIC( 8, 108, 0xff, 0x00, 0x00, "dac_rbqe",  0, 0x00),
   // dup 109..115
    MAKE_AIC( 8, 116, 0xff, 0x00, 0x7f, "dac_rbqf_h",0, 0x7f),   // 190=8.116
    MAKE_AIC( 8, 117, 0xff, 0x00, 0xff, "dac_rbqf_l",0, 0xff),
    MAKE_AIC( 8, 118, 0xff, 0x00, 0x00, "dac_rbqf",  0, 0x00),
   // dup 119..125
    MAKE_AIC( 8, 126, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 8, 127, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
// ********** AIC3100 PAGE 9
// pg,reg, W1_allowed, W1_required, reset_val,  "name",   prev_val, curr_val
    MAKE_AIC( 9,   0, 0x00, 0x00, 0x09, "psr9",      0, 0x09),   // 195=9.0   9:0 reads as 0x09
    MAKE_AIC( 9,   1, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC( 9,   2, 0xff, 0x00, 0x7f, "ldac_iir_h",0, 0x7f),
    MAKE_AIC( 9,   3, 0xff, 0x00, 0xff, "ldac_iir_l",0, 0xff),
    MAKE_AIC( 9,   4, 0xff, 0x00, 0x00, "ldac_iir",  0, 0x00),
   // dup 5..7
    MAKE_AIC( 9,   8, 0xff, 0x00, 0x7f, "rdac_iir_h",0, 0x7f),   // 200=9.8
    MAKE_AIC( 9,   9, 0xff, 0x00, 0xff, "rdac_iir_l",0, 0xff),
    MAKE_AIC( 9,  10, 0xff, 0x00, 0x00, "rdac_iir",  0, 0x00),
   // dup 11..13
    MAKE_AIC( 9,  14, 0xff, 0x00, 0x7f, "drc_hn0_h", 0, 0x7f),
    MAKE_AIC( 9,  15, 0xff, 0x00, 0xf7, "drc_hn0_l", 0, 0xf7),
    MAKE_AIC( 9,  16, 0xff, 0x00, 0x80, "drc_hn1_h", 0, 0x80),
    MAKE_AIC( 9,  17, 0xff, 0x00, 0x09, "drc_hn1_l", 0, 0x09),
    MAKE_AIC( 9,  18, 0xff, 0x00, 0x7f, "drc_hd1_h", 0, 0x7f),
    MAKE_AIC( 9,  19, 0xff, 0x00, 0xef, "drc_hd1_l", 0, 0xef),
    MAKE_AIC( 9,  20, 0xff, 0x00, 0x00, "drc_ln0_h", 0, 0x00),
    MAKE_AIC( 9,  21, 0xff, 0x00, 0x11, "drc_ln0_l", 0, 0x11),   // 210=9.21
    MAKE_AIC( 9,  22, 0xff, 0x00, 0x00, "drc_ln1_h", 0, 0x00),
    MAKE_AIC( 9,  23, 0xff, 0x00, 0x11, "drc_ln1_l", 0, 0x11),
    MAKE_AIC( 9,  24, 0xff, 0x00, 0x7f, "drc_ld1_h", 0, 0x7f),
    MAKE_AIC( 9,  25, 0xff, 0x00, 0xde, "drc_ld1_l", 0, 0xde),
    MAKE_AIC( 9,  26, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   //215 // reserved-- reset = XX, can1=0x00, must1=0xff
   // dup 27..127
// ********** AIC3100 PAGE 12
// pg,reg, W1_allowed, W1_required, reset_val,  "name",   prev_val, curr_val
    MAKE_AIC(12,  0, 0x00, 0x00, 0x0c, "psr12",      0, 0x0c),   // 216=12.0   12:0 reads as 0x0c
    MAKE_AIC(12,  1, 0x00, 0xff, 0x00, "rsv",        0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC(12,  2, 0xff, 0x00, 0x7f, "ldac_bqa_h", 0, 0x7f),
    MAKE_AIC(12,  3, 0xff, 0x00, 0xff, "ldac_bqa_l", 0, 0xff),
    MAKE_AIC(12,  4, 0xff, 0x00, 0x00, "ldac_bqa",   0, 0x00),   // 220
   // dup 5..11
    MAKE_AIC(12, 12, 0xff, 0x00, 0x7f, "ldac_bqb_h", 0, 0x7f),
    MAKE_AIC(12, 13, 0xff, 0x00, 0xff, "ldac_bqb_l", 0, 0xff),
    MAKE_AIC(12, 14, 0xff, 0x00, 0x00, "ldac_bqb",   0, 0x00),
   // dup 15..21
    MAKE_AIC(12, 22, 0xff, 0x00, 0x7f, "ldac_bqc_h", 0, 0x7f),
    MAKE_AIC(12, 23, 0xff, 0x00, 0xff, "ldac_bqc_l", 0, 0xff),
    MAKE_AIC(12, 24, 0xff, 0x00, 0x00, "ldac_bqc",   0, 0x00),
   // dup 25..31
    MAKE_AIC(12, 32, 0xff, 0x00, 0x7f, "ldac_bqd_h", 0, 0x7f),
    MAKE_AIC(12, 33, 0xff, 0x00, 0xff, "ldac_bqd_l", 0, 0xff),
    MAKE_AIC(12, 34, 0xff, 0x00, 0x00, "ldac_bqd",   0, 0x00),
   // dup 35..41
    MAKE_AIC(12, 42, 0xff, 0x00, 0x7f, "ldac_bqe_h", 0, 0x7f),   // 230
    MAKE_AIC(12, 43, 0xff, 0x00, 0xff, "ldac_bqe_l", 0, 0xff),
    MAKE_AIC(12, 44, 0xff, 0x00, 0x00, "ldac_bqe",   0, 0x00),
   // dup 45..51
    MAKE_AIC(12, 52, 0xff, 0x00, 0x7f, "ldac_bqf_h", 0, 0x7f),
    MAKE_AIC(12, 53, 0xff, 0x00, 0xff, "ldac_bqf_l", 0, 0xff),
    MAKE_AIC(12, 54, 0xff, 0x00, 0x00, "ldac_bqf",   0, 0x00),
   // dup 55..61
    MAKE_AIC(12, 62, 0x00, 0xff, 0x00, "rsv",        0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC(12, 63, 0x00, 0xff, 0x00, "rsv",        0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC(12, 64, 0xff, 0x00, 0x00, "pga_gn_h",   0, 0x00),
    MAKE_AIC(12, 65, 0xff, 0x00, 0x00, "pga_gn_l",   0, 0x00),
    MAKE_AIC(12, 66, 0xff, 0x00, 0x7f, "dac_rbqa_h", 0, 0x7f),   // 240
    MAKE_AIC(12, 67, 0xff, 0x00, 0xff, "dac_rbqa_l", 0, 0xff),
    MAKE_AIC(12, 68, 0xff, 0x00, 0x00, "dac_rbqa",   0, 0x00),
   // dup 69..75
    MAKE_AIC(12, 76, 0xff, 0x00, 0x7f, "dac_rbqb_h", 0, 0x7f),
    MAKE_AIC(12, 77, 0xff, 0x00, 0xff, "dac_rbqb_l", 0, 0xff),
    MAKE_AIC(12, 78, 0xff, 0x00, 0x00, "dac_rbqb",   0, 0x00),
   // dup 79..85
    MAKE_AIC(12, 86, 0xff, 0x00, 0x7f, "dac_rbqc_h", 0, 0x7f),
    MAKE_AIC(12, 87, 0xff, 0x00, 0xff, "dac_rbqc_l", 0, 0xff),
    MAKE_AIC(12, 88, 0xff, 0x00, 0x00, "dac_rbqc",   0, 0x00),
   // dup 89..95
    MAKE_AIC(12, 96, 0xff, 0x00, 0x7f, "dac_rbqd_h", 0, 0x7f),
    MAKE_AIC(12, 97, 0xff, 0x00, 0xff, "dac_rbqd_l", 0, 0xff),   // 250
    MAKE_AIC(12, 98, 0xff, 0x00, 0x00, "dac_rbqd",   0, 0x00),
   // dup 99..105
    MAKE_AIC(12, 106, 0xff, 0x00, 0x7f, "dac_rbqe_h",0, 0x7f),
    MAKE_AIC(12, 107, 0xff, 0x00, 0xff, "dac_rbqe_l",0, 0xff),
    MAKE_AIC(12, 108, 0xff, 0x00, 0x00, "dac_rbqe",  0, 0x00),
   // dup 109..115
    MAKE_AIC(12, 116, 0xff, 0x00, 0x7f, "dac_rbqf_h",0, 0x7f),
    MAKE_AIC(12, 117, 0xff, 0x00, 0xff, "dac_rbqf_l",0, 0xff),
    MAKE_AIC(12, 118, 0xff, 0x00, 0x00, "dac_rbqf",  0, 0x00),
   // dup 119..125
    MAKE_AIC(12, 126, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC(12, 127, 0x00, 0xff, 0x00, "rsv",       0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
// ********** AIC3100 PAGE 13
// pg,reg, W1_allowed, W1_required, reset_val,  "name",   prev_val, curr_val
    MAKE_AIC(13,  0, 0x00, 0x00, 0x0d, "psr13",      0, 0x0d),   // 260=13.0    13:0 reads as 0x0d
    MAKE_AIC(13,  1, 0x00, 0xff, 0x00, "rsv",        0, 0x00),   // reserved-- reset = XX, can1=0x00, must1=0xff
    MAKE_AIC(13,  2, 0xff, 0x00, 0x7f, "ldac_iir_h", 0, 0x7f),
    MAKE_AIC(13,  3, 0xff, 0x00, 0xff, "ldac_iir_l", 0, 0xff),
    MAKE_AIC(13,  4, 0xff, 0x00, 0x00, "ldac_iir",   0, 0x00),
   // dup 5..7
    MAKE_AIC(13,  8, 0xff, 0x00, 0x7f, "rdac_iir_h", 0, 0x7f),
    MAKE_AIC(13,  9, 0xff, 0x00, 0xff, "rdac_iir_l", 0, 0xff),
    MAKE_AIC(13, 10, 0xff, 0x00, 0x00, "rdac_iir",   0, 0x00),
   // dup 11..13
    MAKE_AIC(13, 14, 0xff, 0x00, 0x7f, "h_drc_h",    0, 0x7f),
    MAKE_AIC(13, 15, 0xff, 0x00, 0xf7, "h_drc_l",    0, 0xf7),
    MAKE_AIC(13, 16, 0xff, 0x00, 0x80, "h_drc",      0, 0x80),   // 270
    MAKE_AIC(13, 17, 0xff, 0x00, 0x09, "h_drc",      0, 0x09),
    MAKE_AIC(13, 18, 0xff, 0x00, 0x7f, "h_drc_h",    0, 0x7f),
    MAKE_AIC(13, 19, 0xff, 0x00, 0xef, "h_drc_l",    0, 0xef),
    MAKE_AIC(13, 20, 0xff, 0x00, 0x00, "h_drc",      0, 0x00),
    MAKE_AIC(13, 21, 0xff, 0x00, 0x11, "h_drc",      0, 0x11),
    MAKE_AIC(13, 22, 0xff, 0x00, 0x00, "h_drc",      0, 0x00),
    MAKE_AIC(13, 23, 0xff, 0x00, 0x11, "h_drc",      0, 0x11),
    MAKE_AIC(13, 24, 0xff, 0x00, 0x7f, "h_drc_h",    0, 0x7f),
    MAKE_AIC(13, 25, 0xff, 0x00, 0xde, "h_drc_l",    0, 0xde),
    MAKE_AIC(13, 26, 0x00, 0xff, 0x00, "rsv",        0, 0x00),   // 280 // reserved-- reset = XX, can1=0x00, must1=0xff

// dup 27..127
    MAKE_AIC(16,   0, 0x00, 0x00, 0x00, "LAST_ENTRY",0, 0x00)   // 281=16.0    last entry marker
    // @formatter:on
};
...
uint8_t aicGetReg(int idx) {
    ...
    CDC_DBG_LOG_IF( (WatchReg < 0 || idx == WatchReg), "Rd %d:%02d == 0x%02x \n", pg, reg, val);
    return val;
}
...
void aicSetReg(int idx, uint8_t val) {
    ...
    aicSetCurrPage(pg);
    CDC_DBG_LOG_IF( (WatchReg < 0 || idx == WatchReg), "Wr %d:%02d <- 0x%02x \n", pg, reg, val);
    ...
    if ( chkVal != val ) {
      ...
      CDC_DBG_LOG( "Read after write mismatch %d:%02d wr 0x%02x rd 0x%02x \n", pg, reg, val, chkVal );
    }
...
void aicSet16Bits(int idx, uint16_t val) {  // write (val>>8) to idx, (val & 0xFF) to idx+1
    uint8_t pg = codec_regs[idx].pg;
    uint8_t reg = codec_regs[idx].reg;
    CDC_DBG_LOG_IF( (codec_regs[idx + 1].pg != pg || codec_regs[idx + 1].reg != reg + 1), "! set16: i%d/%d not paired\n", idx, idx + 1);

    aicSetCurrPage(pg);

    CDC_DBG_LOG_IF( (WatchReg < 0 || idx == WatchReg), "Wr %d:%02d/%02d <- 0x%04x \n", pg, reg, reg + 1, val);
    ...
    CDC_DBG_LOG_IF( (chkVal != val), "! Read after write mismatch  %d:%02d_%02d wr 0x%04x rd 0x%04x \n", pg, reg,reg+1, val, chkVal );
#endif // VERIFY_WRITTENDATA
        ...
        if (regDef.reg != 0) {
            ...
            if (changed) {
                CDC_DBG_LOG_IF( (valnm == NULL), "R%d.%3d: %12s = 0x%02x %s (0x%02x)\n", regDef.pg, regDef.reg, regDef.name, val, rst,
                           regDef.prev_val);
                CDC_DBG_LOG_IF( (valnm != NULL),"R%d.%3d: %12s = 0x%02x = %s %s (0x%02x)\n", regDef.pg, regDef.reg, regDef.name, val, valnm,
                           rst, regDef.prev_val);
            }
...
void cdc_RecordEnable(bool enable) {
    ...
    */
      ...
      CDC_DBG_LOG_IF( (cnt == MAX_ADC_PWR_WAIT), "! ADC failed to turn off" );
...
void verifyCodecReg(int idx, int pg, int reg) {       // verify that codec_regs[idx] is entry for pg : reg
    ...
    CDC_DBG_LOG_IF( (cr_pg != pg || cr_reg != reg), "! v_codec_reg!: idx=%d  %d:%d != %d:%d \n", idx, pg, reg, cr_pg, cr_reg);
}
#endif
...
//  memset( &akC, 0, sizeof( AK4637_Registers ));
    ...
    CDC_DBG_LOG("2 AIC SftReset %d..%d \n", st, tbTimeStamp());
...
void cdc_ClocksOff(void) {                                      // properly shut down ADC, DAC, clock tree, & PLL
    ...
    CDC_DBG_LOG_IF( (cnt == MAX_DAC_PWR_WAIT), "! DACs failed to turn off" );
    ...
    codecClockFreq = 0;
    CDC_DBG_LOG( "2 ClocksOff \n");
    showCdcRegs( false, false );
    CDC_DBG_LOG( "2 AIC_clksoff %d \n", tbTimeStamp() );
#endif
}
...
void cdc_SetVolume(uint8_t Volume) {
    ...
    CDC_DBG_LOG( "2 cdcSetVol v=%d cdcV=0x%x \n", v, cdcFmtVolume );
...
void cdc_SetMute(bool muted) {                                  // true => enable mute on codec  (audio)
    ...
    if ( muted ){   // enable mute -- both channels
      ...
      CDC_DBG_LOG( "2 AIC mute: SpkrAmp, L&R mutes on \n" );
      ...
      CDC_DBG_LOG( "2 AIC unmute: SpkrAmp & L mutes off, SpkrAmp=6dB \n" );
CDC_DBG_LOG( "2 AIC_unmute %d \n", tbTimeStamp() );
    }
#endif
...
void cdc_SetMasterFreq(
        int freq) {                              // set AK4637 to MasterMode, 12MHz ref input to PLL, audio @ 'freq', start PLL  (i2s_stm32f4xx)
#if defined( AIC3100 )
...
//        case  8000: PLL=92; MDAC=90; NDAC= 2; break;
    ...
    aicSetReg(    P0_R63_DAC_Datapath_SETUP,  0x90 );     // P0_R63: PwrLDAC: 1  PwrRDAC: 0  LDACleft: 01  RDACoff: 00  DACvol1step: 00
    CDC_DBG_LOG( "2 AIC: BCLK = CLK / %d / %d / %d = 32* %d \n", NDAC, MDAC, 2, freq );
//    showCdcRegs( false, false );
