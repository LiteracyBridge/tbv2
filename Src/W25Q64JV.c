...
extern ARM_DRIVER_FLASH ARM_Driver_Flash_(
DRIVER_FLASH_NUM );                 // export driver control block for flash

static ARM_FLASH_INFO               FlashInfo          = {                             // Flash Information
        NULL,         // no flash sector info
        FLASH_SECTOR_COUNT, FLASH_SECTOR_SIZE, FLASH_PAGE_SIZE, FLASH_PROGRAM_UNIT, FLASH_ERASED_VALUE,
...
static int32_t getReg( uint8_t cmd, uint8_t *sreg ) {                                                // read Status 1
    ...
    while (ptrSPI->GetDataCount() != 1) {
        waitcnt++;
        if ( waitcnt > MAX_WAIT ) return ARM_DRIVER_ERROR;
    }
...
static bool WaitWhileBusy() {                                                // read Status 1 until Status1.BUSY goes false
    ...
    while (ptrSPI->GetDataCount() != 1) {
        waitcnt++;
        if ( waitcnt > MAX_WAIT ) return false;
    }
...
static bool SendWriteCommand( uint8_t cmd ) {                                // send write enable, then cmd
    ...
    while (( sentCnt = ptrSPI->GetDataCount()) != cnt) {
        waitcnt++;
        if ( waitcnt > MAX_WAIT ) {
            printf( "! NOR SPI timeout: sentCnt = %d\n", sentCnt );
            return false;
        }
    }
    if ( cnt == 1 ) {
    ...
    switch ((int32_t) state) {
        ...
        case ARM_POWER_FULL:
            ...
            if (( Flags & FLASH_POWER) == 0U ) {
                ...
                status = ptrSPI->Control(
                        ARM_SPI_MODE_MASTER | ARM_SPI_CPOL0_CPHA0 | ARM_SPI_DATA_BITS( 8 ) | ARM_SPI_MSB_LSB |
                        ARM_SPI_SS_MASTER_SW, SPI_BUS_SPEED);
                if ( status != ARM_DRIVER_OK ) return ARM_DRIVER_ERROR;
                FlashStatus.busy  = 0U;
                FlashStatus.error = 0U;
                Flags |= FLASH_POWER;
