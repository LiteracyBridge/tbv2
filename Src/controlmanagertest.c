...
void writeCSM(){			// write current CSM to tbook_csm.c
	char sv[ padLEN+4 ]; 
	FILE * f = tbOpenWrite( "tbook_csm.c" );
	const char *aNms[] = {
	...
	fprintf( f, "//TBook Control State Machine   tbook_csm.c \n" );
  ...
  fprintf( f, "};  // TB_Config \n\n" );
	...
	fprintf( f, "const int   nCSMstates = %d;  // TBook state machine definition \n", nCSMstates );
	
	for ( int i=0; i<nCSMstates; i++ ){
		csmState * st = TBookCSM[i];
	  fprintf( f, "const csmState %s =  // TBookCMS[%d] \n",   st->nm, i );
		fprintf( f, "  { (TknID)(short)0, \"%s\", { \n    ", st->nm );
