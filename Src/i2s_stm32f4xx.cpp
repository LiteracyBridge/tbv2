...
static void I2S3_ClockEnable(bool enab) {                // enable/disable I2S3 clock generator -- as base clock for AK4637 codec
    ...
    if (enab) {
        ...
        spi->I2SCFGR = I2S_MODE |
                       I2S_CFG_MASTER_RX;  // I2Smode Master Receive (0x5400) ASTREN=0 I2SSTD=0 CKPOL=0 DATLEN=00 CHLEN=0
...
static int32_t I2S_Configure(I2S_RESOURCES *i2s, uint32_t freq, bool monoMode, bool slaveMode, bool xmt) {    // set audio frequency
    ...
    if (slaveMode) { // USE odec PLL to generate accurate clock -- I2S3 generates 12MHz ref clock on I2S3_MCK
        ...
        i2s_inst->I2SCFGR =
                I2S_MODE | (xmt ? I2S_CFG_SLAVE_TX : I2S_CFG_SLAVE_RX);  // I2SMode & I2SCFG = Slave xmt or rcv
...
static int32_t I2S_Initialize(ARM_SAI_SignalEvent_t cb_event,
                              I2S_RESOURCES *i2s) {              // init I2S hardware resources
...
static int32_t I2S2only_Send(const void *data, uint32_t nSamples, I2S_RESOURCES *i2s) {           // start data transmit
    ...
    if (info->status
            .tx_busy) {                    // FOR DMA Double Buffering:  allow multiple calls to Send, to keep M0AR & M1AR
    ...
    cfg |= i2s->tx_dma_chan
            << DMA_SxCR_CHSEL_Pos;                  // SPI2_TX is in DMA1 (Table 30) row for Channel 0, as Stream4
...
static int32_t I2S2ext_Receive(void *data, uint32_t nSamples, I2S_RESOURCES *i2s) {                          // start data receive
    ...
    cfg |= DMA_SxCR_TCIE | DMA_SxCR_TEIE | DMA_SxCR_DMEIE;          // interrupt on complete, error, direct mode error
    cfg |= i2s->rx_dma_chan
            << DMA_SxCR_CHSEL_Pos;              // SPI2ext_RX is in DMA1 (Table 30) row for Channel 3, as Stream3
    cfg |= DMA_SxCR_DBM;                      // double buffer, M0AR first
    dma->CR = cfg;
    ...
    i2s->xmt_inst->I2SCFGR = I2S_MODE |
                             I2S_CFG_SLAVE_TX; // set I2S2 into I2SMode & I2SCFG = Slave xmt --- INSURE THAT EXTERNAL CK GETS ROUTED TO I2S2ext
...
static int32_t I2S_Control(uint32_t control, uint32_t arg1, uint32_t arg2, I2S_RESOURCES *i2s) {  // control I2S Interface.
    ...
    if (req == ARM_SAI_CONFIGURE_TX | req == ARM_SAI_CONFIGURE_RX) {
        ...
        const uint32_t reqMSK = ARM_SAI_SYNCHRONIZATION_Msk |
                                ARM_SAI_PROTOCOL_Msk |
                                ARM_SAI_DATA_SIZE_Msk;   // mask for fields that must match supported values
...
void I2S_TX_DMA_Complete(uint32_t intReqs, const I2S_RESOURCES *i2s) {                 // handle TX complete
    ...
    const int DMA_ERROR_MSK = DMA_LISR_TEIF0 |
                              DMA_LISR_DMEIF0 |
                              DMA_LISR_FEIF0;  // mask for transfer error, direct mode error & fifi error
